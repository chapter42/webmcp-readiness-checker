...
const CHROME_DOCS = 'https://developer.chrome.com/docs/ai/webmcp';

/** Spec revision this rubric was last checked against. */
const SPEC_AS_OF = 'July 2026';

/**
 * Extension version, read from the manifest so the report footer can never
 * drift from the shipped version (it was hardcoded to 0.1.4 through three
 * releases).
 * @returns {string}
 */
...
const DOC_LINKS = {
  ...
  model_context_api: {
    label: 'Chrome for Developers — WebMCP Imperative API',
    url: `${CHROME_DOCS}/imperative-api`,
  },
  agent_submit_handling: {
    label: 'Chrome for Developers — WebMCP Declarative API',
    url: `${CHROME_DOCS}/declarative-api`,
  },
  tool_feedback: {
    label: 'WebMCP Spec — Declarative API Explainer',
    url: `${WEBMCP_REPO}/blob/main/declarative-api-explainer.md`,
  },
  annotations: {
    label: 'Chrome for Developers — WebMCP tool security',
    url: `${CHROME_DOCS}/secure-tools`,
  },
  jsonld: {
  ...
  webmcp_manifest: {
    // Not part of the WebMCP draft — a pre-navigation discovery convention
    // that emerged in the community. Scored, but labelled honestly.
    label: 'llms.txt / .well-known discovery (community convention, not in the WebMCP draft)',
    url: 'https://llmstxt.org/',
  },
  robots_txt: {
...
function generateMarkdownReport(scanData, fixSnippets) {
  ...
  lines.push('');
  lines.push(`> Based on the WebMCP spec as of ${SPEC_AS_OF} (W3C Draft Community Group Report — not standards-track).`);
  lines.push('> Chrome runs a WebMCP origin trial from Chrome 149-156. `navigator.modelContext` is');
  lines.push('> deprecated as of Chromium 150 in favour of `document.modelContext`.');
  lines.push('');
  ...
  lines.push('---');
  lines.push(`_Generated by WebMCP Readiness Checker v${extensionVersion()} — rubric based on the [WebMCP draft spec](${WEBMCP_SPEC}) as of ${SPEC_AS_OF}._`);
...
function collectFailingSnippetKeys(scanData, fixSnippets) {
  ...
  for (const cat of Object.values(cats)) {
    ...
    for (const sig of signals) {
      ...
      if (sigKey.includes('model_context') || sigKey.includes('imperative')
          || sigKey.includes('tools_exposed')) matched.add('model_context_api');
      if (sigKey.includes('agent_submit') || sigKey.includes('respondwith')) matched.add('agent_submit_handling');
...
function docLinkForSignal(signalName) {
  ...
  if (key.includes('agent_submit') || key.includes('respondwith')) return DOC_LINKS.agent_submit_handling;
  if (key.includes('annotation')) return DOC_LINKS.annotations;
  if (key.includes('tools_exposed') || key.includes('model_context')
      || key.includes('imperative') || key.includes('registertool')) return DOC_LINKS.model_context_api;
...
function recDocLink(rec) {
  ...
  const keywords = [
    ...
    ['robots', 'robots_txt'],
    ['document.modelcontext', 'model_context_api'],
    ['navigator.modelcontext', 'model_context_api'],
    ['registertool', 'model_context_api'],
    ['imperative', 'model_context_api'],
    ['respondwith', 'agent_submit_handling'],
    ['agentinvoked', 'agent_submit_handling'],
    ['agent submit', 'agent_submit_handling'],
